                          ...
                          npy_bool userdef)
{
    ...
    if (res < 0) {
        ...
        // PyArray_StringDType's scalar is str which we allow because it
        // doesn't participate in DType inference, so don't add it to the
 ...
 */
NPY_NO_EXPORT int
npy_cast_raw_scalar_item(PyArray_Descr *from_descr, char *from_item,
...
PyArray_Pack(PyArray_Descr *descr, void *item, PyObject *value)
{
    ...
    if (NPY_UNLIKELY(descr->type_num == NPY_OBJECT)) {
        ...
        return PyDataType_GetArrFuncs(descr)->setitem(value, item,
                                                      &arr_fields);
    }
    ...
    if (DType == (PyArray_DTypeMeta *)Py_None && PyArray_CheckExact(value) &&
        ...
        return npy_cast_raw_scalar_item(PyArray_DESCR(arr), PyArray_BYTES(arr),
                                        descr, item);
    }
    ...
    if (PyDataType_GetArrFuncs(tmp_descr)->setitem(value, data, &arr_fields) <
        0) {
        PyObject_Free(data);
        Py_DECREF(tmp_descr);
        return -1;
...
static int
update_shape(int curr_ndim, int *max_ndim, npy_intp out_shape[], int new_ndim,
...
PyArray_AdaptDescriptorToArray(PyArrayObject *arr, PyArray_DTypeMeta *dtype,
                               PyArray_Descr *descr)
{
        ...
        int copy)
{
    ...
    if (PyArray_Check(obj)) {
        ...
        arr = (PyArrayObject *)_array_from_array_like(
                obj, requested_descr, 0, NULL, copy, &was_copied_by__array__);
        if (arr == NULL) {
            return -1;
        }
...
force_sequence_due_to_char_dtype:
    ...
    for (Py_ssize_t i = 0; i < size; i++) {
        max_dims = PyArray_DiscoverDTypeAndShape_Recursive(
                ...
                coercion_cache_tail_ptr, fixed_DType, flags, copy);
 ...
 */
NPY_NO_EXPORT PyObject *
_discover_array_parameters(PyObject *NPY_UNUSED(self), PyObject *const *args,
                           Py_ssize_t len_args, PyObject *kwnames)
{
    ...
    if (npy_parse_arguments("_discover_array_parameters", args, len_args,
                            kwnames, "", NULL, &obj, "|dtype",
    ...
    int ndim = PyArray_DiscoverDTypeAndShape(
            obj, NPY_MAXDIMS, shape, &coercion_cache, dt_info.dtype,
            dt_info.descr, (PyArray_Descr **)&out_dtype, 0, NULL);
